...
const ContactForm = () => {
    ...
    const sendData = () => {
        ...
        !store.editContact ? actions.sendData({ name: fullName, email: email, phone: phone, address: addr }):
        actions.updateContact({ id: store.editContact.contactToEdit.id, name: fullName, email: email, phone: phone, address: addr });
        navigate('/')
    }
    ...
    useEffect(() => {
        if (store.editContact && store.editContact.contactToEdit) {
            setFullName(store.editContact.contactToEdit.name || '');
            setEmail(store.editContact.contactToEdit.email || '');
            setPhone(store.editContact.contactToEdit.phone || '');
            setAddr(store.editContact.contactToEdit.address || ''); 
        } else {
            setFullName('');
            setEmail('');
            setPhone('');
